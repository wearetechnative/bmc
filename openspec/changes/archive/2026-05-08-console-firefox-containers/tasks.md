...
- [x] 2.4 When `firefoxContainers = true`: call `firefox "ext+granted-containers:name=<profile>&url=<url>&color=blue&icon=circle"` instead of `xdg-open`
...
- [x] 5.1 With `firefox_containers = false`: console opens via `xdg-open` as before
- [x] 5.2 With `firefox_containers = true`: `firefox "ext+granted-containers:name=<profile>&url=<url>..."` opens in a container tab
- [x] 5.3 With `firefox_containers = true` and firefox not in PATH: clear error message shown

...
## Purpose
This specification defines the behavior of the `bmc ec2scheduler` command, which provides convenient management of the `InstanceScheduler` tag on EC2 instances. The command allows users to toggle between enabled and disabled states to temporarily prevent or allow automatic instance scheduling.

# mfa-per-profile-totp-script Specification

## Purpose
Defines requirements for per-source-profile TOTP script configuration in BMC. This capability allows different source profiles to use different TOTP scripts, with a fallback to the global `totp_script` when no profile-specific script is configured.

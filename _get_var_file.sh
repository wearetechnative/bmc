...
TF_VARS=($(find "${base_directory}" -type f -not -name "*example*"  -name "*.tfvars*" |grep -v ".terraform"  |sort))
TF_VARS_BASE=($(find "${base_directory}" -type f -not -name "*example*"  -name "*.tfvars*" -exec basename {} \; |grep -v ".terraform" |sort))

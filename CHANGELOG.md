...
- **`bmc console --select` forces profile selection**: Running `bmc console --select` opens the interactive profile selector, even when `AWS_PROFILE` is already set in the environment.
